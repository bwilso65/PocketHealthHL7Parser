...
{
    public sealed record Expectation(string File, int Http, string Ack, string DbStatus, string? RejectionCode, int Reports);

    // HTTP 200 for every one of these: each was durably stored. The verdict is in the ACK / DB status.
    public static readonly Expectation[] Matrix =
    [
        new("01_oru_valid_minimal.hl7",     200, "AA", "accepted",  null,                        1),
        new("02_oru_valid_01.hl7",          200, "AA", "accepted",  null,                        1),
        new("03_oru_valid_02.hl7",          200, "AA", "accepted",  null,                        1),
        new("04_oru_duplicate_retry.hl7",   200, "AA", "duplicate", null,                        0),
        new("05_malformed.hl7",             200, "AR", "rejected",  "UNPARSEABLE",               0),
        new("06_malformed_double_msh.hl7",  200, "AR", "rejected",  "MULTIPLE_MSH",              0),
        new("07_malformed_truncated.hl7",   200, "AE", "rejected",  "REQUIRED_FIELD_MISSING",    0),
        new("08_adt_wrong_type.hl7",        200, "AR", "rejected",  "UNSUPPORTED_MESSAGE_TYPE",  0),
    ];
    ...
    public async Task All_samples_in_order_produce_the_documented_outcomes()
    {
        ...
        foreach (var e in Matrix)
        {
            ...
            Check("HTTP status", e.Http, result.StatusCode);
            Check("MSA-1", e.Ack, result.AckCode);
            Check("messages.status", e.DbStatus, (string?)row?.status);
        ...
        // Nothing is ever dropped: one messages row per POST, and every row has the raw bytes.
    ...
    public async Task Same_control_id_from_a_different_facility_is_not_a_duplicate()
    {
        ...
        Assert.Equal("accepted", (string)server.Message(other.MessageId)!.status);
